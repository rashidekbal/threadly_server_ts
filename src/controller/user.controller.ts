...
async function getUserController(req:express.Request, res:express.Response) {
  ...
  if (!useridtofetch) return res.status(400).json(new ApiError(400, apiErrorType.API_ERROR,new ErrorDetails("please provide valid userid"))); // Return 400 Bad Request if no user ID is provided
    ...
    return res.status(500).json(new ApiError(500,apiErrorType.API_ERROR ,new ErrorDetails(null))); // Return 500 Internal Server Error on failure
  }
}

async function getUserByUUidController(req:express.Request, res:express.Response) {
  ...
  if (!uuid) return res.status(400).json(new ApiError(400, apiErrorType.API_ERROR,new ErrorDetails("please provide a valid uuid"))); // Return 400 Bad Request if no user ID is provided
...
//  */
async function getMyDataController(req:express.Request, res:express.Response) {
  try {
      let userid = req.body.userid; // Extract user ID from the request
